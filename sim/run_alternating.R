...
run_alt<-function(scenario,sc){
  ...
  for(rep in 1:n_sim){
    ...
    te<-if(sc==4){s1<-X[,"S1"];2*((-1.07<=s1&s1<1.07)&(-1.07<=X[,"S2"]&X[,"S2"]<1.07))-1
    }else{s<-X[,"S1"];2*ifelse(s>=0.67|(-0.67<=s&s<0),1,0)-1}
    Zb<--Zb^2
    ...
    # Original CAVBoost
    fo<-tryCatch(train_rmst_cavboost(tr,tr$time,tr$status,tau,eta=0.05,max_depth=3,nr=nr,covars=NULL),error=function(e)NULL)
    if(is.null(fo)){cat(sprintf("orig fail %s %d\n",scenario,rep));next}
    po<-pred_subgroup(fo,te_df)
    
    # Stratified (Cox on 6 vars only, not all 52)
    ps_cph<-tryCatch(predict(coxph(Surv(time,status)~Z1+Z2+Z3+Z4+S1+S2,data=tr),type="lp"),error=function(e)NULL)
    if(is.null(ps_cph)){ps_te<-rep(0.5,nrow(te_df))}else{
      qq<-stats::quantile(ps_cph,c(0.25,0.5,0.75),na.rm=TRUE)
      st_<-as.numeric(ps_cph<=qq[1])+as.numeric(ps_cph>qq[1]&ps_cph<=qq[2])+as.numeric(ps_cph>qq[2]&ps_cph<=qq[3])+as.numeric(ps_cph>qq[3])
      fs<-tryCatch(train_stratified_cavboost(tr,tr$time,tr$status,tau,stratum=st_,eta=0.1,max_depth=2,nr=nr,covars=NULL),error=function(e)NULL)
      ps_te<-if(!is.null(fs))pred_stratified(fs,te_df)else rep(0.5,nrow(te_df))
    }
    
    # Alternating: build custom eta vector round by round
    ipcw_w<-compute_ipcw_weights(tr$time,tr$status,tau)
    sd_orig<-make_sorted_data(tr$time,tr$status,ipcw_w$weights,tr$trt01p,tau)
    K<-4;sorted_strata<-vector("list",K)
    for(k in 1:K){idx<-which(st_==k);if(length(idx)>=2)sorted_strata[[k]]<-make_sorted_stratum(tr$time[idx],tr$status[idx],ipcw_w$weights[idx],tr$trt01p[idx],tau)}
    eta<-rep(0,n_tr);tree_list<-list()
    for(rd in 1:nr){
      p<-1/(1+exp(-eta));pg<-p*(1-p)
      if(rd%%2==1){
        # Original gradient (global p-weighted KM)
        ps<-p[sd_orig$ord]
        ...
        if(length(et)<2){g<-rep(0,n_tr)}else{
          ...
          for(i in seq_along(et)){
            ...
            ht<-hazard_inc(in_e1,in_r1,ps*sd_orig$ipcw,sd_orig$ipcw,n_tr)
            hc<-hazard_inc(in_e0,in_r0,ps*sd_orig$ipcw,sd_orig$ipcw,n_tr)
            hh1<-hazard_inc(in_e1,in_r1,(1-ps)*sd_orig$ipcw,sd_orig$ipcw,n_tr)
            hh0<-hazard_inc(in_e0,in_r0,(1-ps)*sd_orig$ipcw,sd_orig$ipcw,n_tr)
        ...
        # Stratified gradient
        g<-rep(0,n_tr)
        for(k in 1:K){sd<-sorted_strata[[k]];if(is.null(sd)||sd$n<2)next
          idx_k<-which(st_==k);pk<-p[idx_k]
          rr<-stratum_gradient_components(pk[sd$ord],sd,tau)
          Wk<-sum(pk);g[idx_k]<-pg[idx_k]*(-(rr$d1+rr$d2+Wk*rr$g1[sd$iord]-(length(pk)-Wk)*rr$g2[sd$iord]))
        }
      }
      dt<-xgb.DMatrix(as.matrix(tr[,f]),label=g)
      bt<-xgb.train(params=list(eta=0.05,max_depth=2),data=dt,nrounds=1,verbose=0,objective="reg:squarederror")
    ...
    cmf<-function(p,ll){r<-roc(ll,p,direction="auto");c(AUC=as.numeric(r$auc),Acc=mean((p>.5)==ll),FPR=if(sum(!ll)>0)sum(p>.5&!ll)/sum(!ll)else NA,FNR=if(sum(ll)>0)sum(p<=.5&ll)/sum(ll)else NA)}
    r_o[[rep]]<-cmf(po,l);r_s[[rep]]<-cmf(ps_te,l);r_a[[rep]]<-cmf(pa_te,l)
  }
  ro<-do.call(rbind,r_o[!sapply(r_o,is.null)]);rs<-do.call(rbind,r_s[!sapply(r_s,is.null)]);ra<-do.call(rbind,r_a[!sapply(r_a,is.null)])
  rn<-function(d)list(mean(d[,1],na.rm=T),mean(d[,2],na.rm=T),mean(d[,3],na.rm=T),mean(d[,4],na.rm=T),nrow(d))
...
run_alt("S4_Enclave",4);run_alt("S5_SShaped",5)
