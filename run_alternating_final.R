...
# Cross-fitted prognostic strata. No IPCW.
# Usage: Rscript run_alternating_final.R  (run from repo root)
...
run_alt <- function(scenario, sc) {
  ...
  for (rep in 1:n_sim) {
    ...
    # Stratified with cross-fitted prognostic strata
    ...
    # Alternating rounds: original (odd) <-> stratified (even), no IPCW
    ...
    for (rd in 1:nr) {
      ...
      if (rd %% 2 == 1) {
        ...
        # Stratified gradient (cross-fitted strata)
        g <- rep(0, n_tr)
        for (k in 1:K) {
          sd <- sorted_strata[[k]]
  ...
  for (lst in list(list("Orig", ro), list("Strat_CF", rs), list("Alternating", ra))) {
